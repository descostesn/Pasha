...
normAndSubtractWIG=function(wigFileList, inputFile=NA, rescaleInput=FALSE, meanBGLevelRescale=c(0,10), subtractInput=TRUE, binSize=50, rescaleIP=TRUE)
{
	...
	for(currentWIG in wigFileList[!is.na(wigFileList)])
	{
		...
		output_name  <-  paste(substr(currentWIG,1,nchar(currentWIG)-4),sep="");
