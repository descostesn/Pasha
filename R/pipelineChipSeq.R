...
# That's why multicore is the best suited (not ideal though), I just use mclapply directly in a very naïve way, trying to minimize the
...
.checkErrorsInFork=function(mclapplyResult)
{
	...
	# The other error finding  mechanism (grep in text messageà is kept for compatibility
...
generatePiled <- function(alignedDataObject, elongationSize, averageReadSize, midPoint=FALSE, reference_chrom_length_vector)
{
...
#                                readLength=qwidth(alnCurrentChrom))
		...
		current_chrom_length <- reference_chrom_length_vector[which(names(reference_chrom_length_vector) == currentChr)];
		...
		piledRLE <- Rle(pileupDouble(start=position(alnCurrentChrom),
						...
						weight=weightParam,
						current_chrom_length = current_chrom_length));
			...
			for(currentIncrArtefactThrEvery in incrArtefactThrEvery[[expName]])
			{
				...
				for(currentElongationSize in elongationSize[[expName]])
				{
					...
					if(any(c(WIGvs,WIGfs,GFF)))
					{
						...
						cat("\n Retrieving length of the reference chromosomes\n");
						cat("\t Reading file: ", annotationGenomeFiles[[1]], "\n");
						reference_info_genome <- readLines(annotationGenomeFiles[[1]]);
						length_reference_chromosomes <- as.numeric(strsplit(reference_info_genome[2], " ")[[1]]);
						names(length_reference_chromosomes) <-  strsplit(reference_info_genome[3], " ")[[1]];
						...
						if(doPiling["orphans"])
						{
							...
							returnList[[expName]][["execTime"]][[paste(programProgressName_elongation, "Piling Orphans",sep=" | ")]]=system.time((piledRleData[["orphans"]]=mclapply(.encapsulateListElementsInEnv(chrSplit_orphanReads), generatePiled, currentElongationSize, averageReadSize, INPUTFilesList[[expName]]$midPoint, length_reference_chromosomes)))
							if(length(elongationSize[[expName]])<2)
							{
						...
						if(doPiling["multiReads"])
						{
							...
							returnList[[expName]][["execTime"]][[paste(programProgressName_elongation, "Piling Multireads",sep=" | ")]] <- system.time((piledRleData[["multiReads"]] <- mclapply(.encapsulateListElementsInEnv(chrSplit_multiLocDataObject), generatePiled, currentElongationSize, averageReadSize, INPUTFilesList[[expName]]$midPoint, length_reference_chromosomes)))
							if(length(elongationSize[[expName]])<2)
							{
